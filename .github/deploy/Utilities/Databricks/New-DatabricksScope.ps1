function New-DatabricksScope {
  ...
  if ($keyVaultDns -and $keyVaultResourceId) {
    databricks secrets create-scope $name --scope-backend-type AZURE_KEYVAULT --resource-id $keyVaultResourceId --dns-name $keyVaultDns --initial-manage-principal users
  }
  else {
    databricks secrets create-scope $name --initial-manage-principal users
  }
}
