...
a delta table with a given schema before carrying out further transformations. Ev
...
- The delta table must use one of the following partitioning sets
  ...
  The capture files will be read from the latest partition in delta and forward, 
    only. (The latest partition will truncated and re-read to ensure complete but 
