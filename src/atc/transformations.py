...
def join_time_series_dataframes(dfPrimary: DataFrame, dfSecondary: DataFrame, startTimeColumnName: str, endTimeColumnName: str, idColumns: List[str], stateColumn: str) -> DataFrame:
    ...
    df_join = df_join.withColumn("dfPri_StateGroupId", F.sum((F.col(f"dfPri_{stateColumn}") != F.lag(F.col(f"dfPri_{stateColumn}")).over(window)).cast("int")).over(window))
    ...
    df_join = df_join.withColumn(f"dfPri_{endTimeColumnName}", F.when((F.col(f"dfPri_{endTimeColumnName}") == F.lead(F.col(f"dfPri_{endTimeColumnName}")).over(window)), F.col(f"dfSec_{endTimeColumnName}")).otherwise(F.col(f"dfPri_{endTimeColumnName}")))
    df_join = df_join.withColumn(f"dfPri_{startTimeColumnName}", F.when((F.col(f"dfPri_{startTimeColumnName}") == F.lag(F.col(f"dfPri_{startTimeColumnName}")).over(window)), F.lag(F.col(f"dfSec_{endTimeColumnName}")).over(window)).otherwise(F.col(f"dfPri_{startTimeColumnName}")))

    # Generating three time segements, one with the state from first the primary dataframe, one with the state from the secondary dataframe and one with the rest of primary dataframe
    # Correcting the end timestamps acording to if there are a matching element in seconday dataframe
    df_join = df_join.withColumn("StartTimeDfPri1", F.col(f"dfPri_{startTimeColumnName}"))
    df_join = df_join.withColumn("EndTimeDfPri1", F.when(F.col(f"dfSec_{stateColumn}").isNotNull(), F.col(f"dfSec_{startTimeColumnName}")).otherwise(F.col(f"dfPri_{endTimeColumnName}")))

    df_join = df_join.withColumn("StartTimeDfSec", F.col(f"dfSec_{startTimeColumnName}"))
    ...
    df_join = df_join.withColumn("EndTimeDfPri2", F.when(F.col(f"dfSec_{stateColumn}").isNotNull(), F.col(f"dfPri_{endTimeColumnName}")).otherwise(None))

    # Collecting segments of timestamps into lists
    df_join = df_join.withColumn("DfPri1", F.when((F.col("StartTimeDfPri1") < F.col("EndTimeDfPri1")), F.array(F.col("StartTimeDfPri1"), F.col("EndTimeDfPri1"))).otherwise(F.array(F.lit(None), F.lit(None))))
    df_join = df_join.withColumn("DfSec", F.when((F.col("StartTimeDfSec") < F.col("EndTimeDfSec")), F.array(F.col("StartTimeDfSec"), F.col("EndTimeDfSec"))).otherwise(F.array(F.lit(None), F.lit(None))))
    df_join = df_join.withColumn("DfPri2", F.when((F.col("StartTimeDfPri2") < F.col("EndTimeDfPri2")), F.array(F.col("StartTimeDfPri2"), F.col("EndTimeDfPri2"))).otherwise(F.array(F.lit(None), F.lit(None))))

    # Unpivot dataframe to move timestamp segemnts from columns to rows
    unpivot_columnn_select = idColumns + [f"dfPri_{stateColumn}"] + [f"dfSec_{stateColumn}"] + ["stack(3, 'DfPri1', DfPri1, 'DfSec', DfSec, 'DfPri2', DfPri2) as (type, data)"]
    ...
    df_join_unpivot = df_join_unpivot.where(F.col(startTimeColumnName).isNotNull() | F.col(endTimeColumnName).isNotNull())

    # Define stateColumn from either df primary or df seconday
    df_join_unpivot = df_join_unpivot.withColumn("tempStateColumn", 
    ...
    df_join_unpivot = df_join_unpivot.dropDuplicates(subset=[startTimeColumnName, endTimeColumnName])
