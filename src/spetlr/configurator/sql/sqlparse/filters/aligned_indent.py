...
from spetlr.configurator.sql.sqlparse import sql
from spetlr.configurator.sql.sqlparse import tokens as T
from spetlr.configurator.sql.sqlparse.utils import indent, offset
...
class AlignedIndentFilter:
    ...
    def nl(self, offset=1):
        ...
        return sql.Token(
            T.Whitespace,
            self.n + self.char * (self._max_kwd_len + offset + indent + self.offset),
        )
    ...
    def _process_case(self, tlist):
        ...
        for i, (cond, value) in enumerate(cases):
            ...
            if cond:
                ws = sql.Token(
                    T.Whitespace, self.char * (max_cond_width - condition_width[i])
                )
    ...
    def _split_kwds(self, tlist):
        ...
        while token:
            ...
            # word as aligner
            if token.match(T.Keyword, self.join_words, regex=True) or token.match(
                T.Keyword, self.by_words, regex=True
            ):
    ...
    def _process_default(self, tlist):
        ...
        for sgroup in tlist.get_sublists():
            ...
            offset_ = (
                3
                if (prev_ and prev_.match(T.Keyword, self.by_words, regex=True))
                else 0
            )
